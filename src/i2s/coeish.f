     ...
     & shbar,shrms,fpilha)
c
c  Determines shift between even and odd interferogram point subsets.
c  This can be non-zero due to mis-sampling of the laser fringe zero-
c  level crossings. Uses only LGEV igram points surrounding ZPD.
c
c  Inputs:
...
c    FPILHA         R*4  Fraction of power in lower half alias
c                        (LSF/laser sampling fraction in spectrum header)
c
c  Theoretical Basis:
c
...
c
c  Since aliasing can bring spurious signal into the domain
c  of interest, carrying the wrong phase, points affected by
c  aliasing must be heavily de-weighted in the averaging
c  process. Fortunately, we can tell from the full igram
c  how much aliasing there will be at each point in the
c  igram-subsets used in the phase different calculation.
c  These aliasing-affected points can then be appropriately
c  de-weighted in the averaging of the phase differences.
c 
c  The raw interferogram is band-limited, but mis-sampled.
c  If we construct a new interferogram comprising of just
c  the odd or even interferogram points, the sampling is 
c  now perfect, but the interferogram is, in general, not
c  band limited. [If it was, then the interferogram size
c  and data rate was a factor two larger than necessary.]
c
c  Implementation Notes:
c
c    Input vector (AC_IGRAM) is unmodified.
c
c    ZPD location assumed to be at AC_IGRAM(LGEV/2+1) which
c    is an odd point as far as THIS subroutine is concerned.
c
...
c  spectra (not by subtracting the phases) to avoid
...
c     & totsa,totsd,
     & tw,tdela,tda2,tdelb,tdb2
      ...
      pi=3.14159265
      tots_lower=-1.d0  ! Avoid compiler warnings
      lgev2=lgev/2
      lgev4=lgev/4
      lasf=15798.03
      dnu=lasf/lgev2
...
c     toti2=0.0d0
      do j=1,lgev
         ...
         wsall0(j)=yy
c         wsall1(j)=yy
         wsoe(ji,iflip)=2*yy !  Double WSOE igram values (half the points)
...
c56    format(2i4,7f12.5)
      tots_full=0.5*(dble(wsall0(1))**2+dble(wsall0(2))**2)  ! DC & Nyquist terms
      do k=1,lgev2-1
...
c         phase1=atan2(wsall1(ki),wsall1(kr))
         if(k.eq.lgev4) tots_lower=tots_full+power0_all/2
         tots_full=tots_full+power0_all
...
c      write(56,*)icall,k,k*dnu,abs(wsall0(2)),.0,abs(wsall1(2)),.0
      ...
      fpilha = sngl( tots_lower/tots_full )
c fraction of power in lower half alias (fpilha: LSF in spectrum header)
      if(fpilha.gt.0.5) then
         kflip=1   !  signal is mainly in lower half of alias
c      write(*,*) 100*tots_lower/tots_full,'% of Signal in lower half of alias'
      else
         kflip=-1  !  signal is mainly in upper half of alias
c      write(*,*)100*(1-tots_lower/tots_full),'% Signal in upper half of alias'
      endif
...
c  damaged by aliasing.
         ...
         phasei=sngl( datan2(ai0,ar0) )
         phasek=sngl( datan2(ai1,ar1) )
...
c     &   sqrt(aliasing0),wt,0.0,0.0,0.0,0.0
      shabar=sngl( tdela/tw )
      sharms=sngl( dsqrt( tda2*tw - tdela**2 )/tw )
      shbbar=sngl(tdelb/tw)
      shbrms=sngl( dsqrt( tdb2*tw - tdelb**2 )/tw )
      den= 1/sharms**2 + 1/shbrms**2
      shbar=sngl( (shabar/sharms**2+shbbar/shbrms**2)/den )
      shrms=sngl(1.d0/dsqrt(den))
