      subroutine smooth_igram(nip,margin,yin,yout,frsp,dclevel,frzpda,
     & pinv,pinl)
c
...
c           margin      I*4      Number of points at ends of igram
c                                within which PINL cannot be located.
...
c  
      implicit none
      integer*4
     & nip,nhw,nwid,i,j,pinl,ii,margin
...
c  which otherwise don't get addressed during the convolution.
      ...
      do j=1,nhw
         ...
         yout(j)=sngl( sinc(xx)+(sinc(xx-pi)+sinc(xx+pi))/2 )
...
c  discontinuities at pinl-nwid and pinl+nwid
      do ii=-nwid,nwid
         ...
         yout(pinl+ii)=(1-fwt)*yout(pinl+ii)+fwt*sngl(dclevel+ii*grad)
      end do
      return
      end
