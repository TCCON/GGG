...
c     Get stuff from text log block (same for both PCDA and PCAT)
        ...
        if(i.eq.13) stop 'Cannot find "Start time"'
...
c     Get other stuff from logblock, PCAT/PCDA dependent
        ...
        fovi=fovi/325
        wsfact=1.00000D0
        hh=hh-10     ! UOW times are UT+10 hours
        ...
        opd=1./resn
c        pout=1020.  !18/9/02 Now read from Bomem binary header
        tout= 25.
        hout= 45.
        if(ivac.le.1)then
            pins=0.1
...
c!!!!!!!!!!!
        snr= 2000.0d0
c:!!!!!!!!!!!!!!!!
c        ss=ss+(nfwdscans-1)*0.375*opd  ! REV runs are twice as fast as FWD
