...
C
          ...
          li(nBand)=liR
          lf(nBand)=lfR
c Trick to skip LM calculation if not enough lines
      if(jmxP.lt.40.or.jmxR.lt.40)li(nband)=55
      if(jmxQ.ge.0.and.jmxQ.lt.40)li(nband)=55
c end skip
          BandFIle(nBand)='S'//CHAR(IsotR+48)//c1//CHAR(lfR+48)//C2//C3
     /       //CHAR(liR+48)//C4//C5
        End If
...
C
C*********************************************************************
      Subroutine ReadW(path_to_wfile)
C*********************************************************************
...
C     
C*********************************************************************
      Subroutine Readlines(path_to_sfile)
C*********************************************************************
...
C     Lines (Cm-1/Atm)
C     BHWAIR : Temperature Dependence Coefficients of HWT0AIR
C     HWT0H2O : H2O-broadened Half-Widths (at 296 K) of the 
C     Lines (Cm-1/Atm)
C     BHWH2O : Temperature Dependence Coefficients of HWT0H2O
...
C     Data of the Lines at Ref Temperature/Pressure
      ...
      Common/GamT0AIR/HWT0AIR(nLmx,nBmx) 
      Common/DTGAMAIR/BHWAIR(nLmx,nBmx) 
      Common/GamT0H2O/HWT0H2O(nLmx,nBmx) 
      Common/DTGAMH2O/BHWH2O(nLmx,nBmx) 
      Common/SHIFT/SHFT(nLmx,nBmx) 
...
c        write(*,*)'Opening:',path_to_sfile(:lp)//Bandfile(iband)//'.dat'
 ...
 2       Read(iFile,1001,End=100)Isot(iBand),Sig(nLineR,iBand),Intens
     ...
     ,  ,HWT0H2O(nLineR,iBand),BHWH2O(nLineR,iBand)
     ,  ,Dipo0(nLineR,iBand),PopuT0(nLineR,iBand)
      DipoT(nLineR,iBand)=DSQRT(Intens/(PopuT0(nLineR,iBand)
     *  *Sig(nLineR,iBand)
     * *(1.D0-dexp(-1.4388*sig(nLineR,iBand)/296.D0))))
      if(TpLine.EQ.'P')then
         Jf(nLineR,iBand)=Ji(nLineR,iBand)-1  !P line
      else 
          if(TpLine.EQ.'Q')then
             Jf(nLineR,iBand)=Ji(nLineR,iBand)  ! Q line
          else
                Jf(nLineR,iBand)=Ji(nLineR,iBand)+1   ! R line
          end if
      end if

1001  Format(2x,i1,f12.6,E10.3,10x,F5.4,5x,f10.4,f4.2,f8.6
     ,  ,50x,A1,I3,38x,F6.4,F5.3,2d16.8)
         nLineR=nLineR+1
...
C*********************************************************************
C*********************************************************************
      Subroutine CompAbsCO2(path_to_sfile,Temp,Ptot,xCO2,xH2O,
     &     SigMin,DSig,Nsig,
     &     AbsV,AbsY,AbsW,MixFull)
C*********************************************************************
...
C     --------------------------------
C     HWT    : HalfWidths of the Lines for the
C              considered humidity, Temperature and Pressure (Cm-1)
...
c      nSig=dInt( ((SigMax-SigMIn)/DSig)+0.5d0 )+1
...
C     
c      write(*,*)' Calling ReadLines..'
      call Readlines(path_to_sfile)     
...
c      write(*,*)' Calling ConvTP...'
         Call ConvTP(iBand,Isot(iBand),nLines(iBand),xH2O,Temp,Ptot,
     &               SigMoy,MixFull)
c      write(*,*)' Called ConvTP.'
...
C     Call the Subroutine for calculation of the Voigt function
                  YY=HWT(iLine)*Cte
                  XX=(Sig(iLine,iBand)+SHFT(iLine,iBand)*Ptot-SigC)*Cte
...
c
                  if(MixFull)then
               IF(dabs(AlphR(iLine)+SigMoy-SigC).le.
     &              (rdmult*GamD))THEN
...
c     Last Corrections on Results
      ...
      do 6 iSig=1,nSig 
         ...
         AbsW(iSig)=AbsW(iSig)-AbsY(iSig)
         AbsY(iSig)=AbsY(iSig)-AbsV(iSig)
 6    Continue     
C     
      Return
      End Subroutine CompAbsCO2
C*********************************************************************
C*********************************************************************
      Subroutine ConvTP(iBand,IsotC,nLineC,xH2O,Temp,Ptot
     ,              ,SigMoy,MixFull)
C*********************************************************************
C     
...
C     Treat all Lines for Temperature    
     ...
     &        *dExp(-Ct*E(iLine,iBand)*(1.d0/Temp-1.d0/T0))
         HWT(iLine)=((1-xH2O)*(HWT0AIR(iLine,iBand)
     &        *(RatioT**BHWAIR(iLine,iBand))))
     +   +(xH2O*(HWT0H2O(iLine,iBand)
     +        *(RatioT**BHWH2O(iLine,iBand))))
...
C
c      write(*,*) 'Calling CalcW...'
      Call CalcW(nLineC,iBand,Temp)
c      write(*,*) 'Called CalcW.'
...
c         write(*,*) 'Calling Eqvlines...'
         Call EqvLines(iBand,nLineC,SigMoy)
c         write(*,*) 'Called Eqvlines...'
      ENDIF
...
C
      ...
      do i=1,nraies
         ...
         enddo
      enddo
      if(li(iband).gt.5.or.lf(iband).gt.5.
     . or.IABS(li(iband)-lf(iband)).gt.1)then
      DO ir=1,nraies
         w(ir,ir)=HWT(ir)
      Enddo
      return
...
C     Using detailed balance
      deltasig=sig(iR,iBand)-sig(iRP,iBand)
      if(dabs(deltasig).lt.1.d-4)deltasig=1.d-4
            sum0=sum0+2.d0*dabs(DipoT(iRP,iBand))
     &           /dabs(DipoT(iR,iBand))*
     &           (1.d0/(deltasig))*( w(iRP,iR) )
      Enddo
...
C -----------------   with the LAPACK Library. To change, do the
C                     following:
C        - TO SWITCH to the IMSL library, remove the commentary 
C          (!!!!!!) AND ADD COMMENTARY characters in the places of  
C          Lapack calling strings (******)
C                    
C     
...
!!!!!!
! Diagonalize with IMSL library    
!
!      Call DEVCCG(nLineC,zOp,nLmx,zVal,zVec,nLmx)
!
! end diagonalization with IMSL library
!!!!!!

******
* Diagonalize with LAPACK library    
*
...
*
* end diagonalization with LAPACK library
******
...
!!!!!!
! Begin inversion with IMSL library    
!
!      Call DLINCG(nLineC,zVec,nLmx,zVecM1,nLmx)
!
! end inversion with IMSL library    
!!!!!!

******
* Begin inversion with LAPACK library    
*
...
*
* end inversion with LAPACK library    
******
...
c 501  Format(1x,'************ PROBLEM !!!! ******************',
c     /       /,1x,'The Inversion of Operator "zVec"',
c     /       /,1x,'By  Routine ',A6,' Called in Present',
c     /       /,1x,'Program by Routine "EqvLines" Has a Problem',
c     /       /,1x,'---> The program is Stopped',///)
      Return
      End
