c  Program diurnret
c  Retrieves VMR profiles from measured slant column abundances
c  and their uncertainties, together with calculated slant paths.
c
c  Input Files:
...
c    Version 4.19 2017-05-20  Created seperate vector, ZVMR, for vmr altitudes
c    Version 4.20 2017-06-30  ??  
c    Version 4.21 2017-08-09  Use (4:8) of specname, instead of (4:10)

      implicit none
      include "../gfit/ggg_int_params.f"
      include "../comn/postproc_params.f"

      integer*4 hilay,i,ieof,ilay,iobs,it,j,kcol,kgas,idum,
     & ldel,lmax,lmin,lnbc,lo,lowlay,ltop,lunr_dc,lunr_ray,
     ...
     & nn,nobs,noly,np,nsza,nspeci,ocltn,neq,ext,naddn
      parameter (lunr_mav=12)  ! runlog.mav file
c      parameter (lun_vmr=13)  ! runlog.vmr output file (dormant) 
      ...
      parameter (lunr_dc=18)   ! diurnal.xxx input files 
      ...
      parameter (mcomm=15)     ! Max Number of comment lines
      parameter (mwin=448)    ! Max Number of gases/windows to be retrieved
...
c
     ...
     & dcfstarr(msza)*8,gas1*32,lavlabel*12000,mavlabel*2248,
     ...
     & next_spectrum_mav*(nchar),ray_format*64,
     & vmrfile*(mfilepath),modfile*80,gasnames(mgas+1)*32,c1*1,
     & gggdir*(mpath),dl*1,     !ggg directory path (GGGPATH?)
     & input_fmt*40,output_fmt*40,addn_lines(maddln)*(mcharhead),cdum*1
c
      logical dcfilexst
c
      real*8 sza,aszai,aszaz,ddum,denom,doty(mobs),dotyi,dotyz,dsdot,dz,
     & afit,cfit,ufit,
     & delz,eorv,ervc,hour(mobs),houri,hourz,oblat(mobs),oblon(mobs),
     & solar_time,tlat(mobs),tlati,tlatz,tlon(mobs),tloni,tlonz,totwt,
     & totwz,totwz2,wt,year(mobs),yeari,yearz,zobs(mobs)
c
      real*4 pins(mobs),tins(mobs),asza_lav(mobs),azim(mobs),
     & d(mlev),d2r,dcf,smfact,mtlat,
     & vmr(mlev,mgas),fuap,rr2,
     & dnight,dsp(mobs),dsunny,
     & dum,dcell,erad,ev(mlev,mwin),fr,asza_ray,
     & p(mlev),pd1(mm,mlev+1),pd2(mm,mlev+1),pobs(mobs),
     & rnorm,run(mobs),scht,slsfin,smoo,sp(mobs,mlev),t(mlev),
     & tab(malt,msza),talt(malt),tinfo,tobs(mobs),hobs(mobs),
     & tsza(msza),twodint,vaa(mlev),
     & vcon(mlev,mwin),dzmin,dzmax,zminwas,
     & vold,vunc(mlev,mwin),wk(mlev),xx,z(mlev),zvmr(mlev),zeff(mobs),
     & zmin(mobs),zz(mm),km2cm,spi

      real*8 ta,tc,tt,tv,tu,tx,ttv,ttx,tverr,verr,vbar,frac,
     &  slcol(mobs,mwin),slerr(mobs,mwin),vaacol,concol,unccol,fdum
...
c     Platform specification:      DG090519
      ...
      idum=mauxcol  ! avoid compiler warnings (unused parameter)
      idum=mcolvsw  ! avoid compiler warnings (unused parameter)
      idum=mcolvav  ! avoid compiler warnings (unused parameter)
      idum=mrow_qc  ! avoid compiler warnings (unused parameter)
      idum=mvmode   ! avoid compiler warnings (unused parameter)

      cdum(1:1) = countfmt(1:1) ! Avoid compiler warning (unused param)

      ray_format='(a,7f11.5,200f11.5)' ! Default, in case not present

      version=' DIURNRET     Version 4.30      2020-06-30   GCT'
      write(6,*)version
c
 ...
 117     format(' Enter Slant Column File (e.g. fai97rat.lav): ',$)
      ...
      lo=lnbc(lavfile)
c==================================================================
      open(lunr_rpt,file='diurnret'//lavfile(lo-3:lo)//'.rpt',
     & status='unknown')
...
c  Read the .lav file header
      open(lunr_lav,file=lavfile,status='old')
      call read_postproc_header(lunr_lav, ncomm, ncollav, nrow, nauxlav,
     & fdum, input_fmt, addn_lines, naddn)
...
c Write .ret file
      ...
      write(lunw_ret,*)ncomm,nauxret+2*ngas,nauxret
      write(lunw_con,*)ncomm,nauxret+2*ngas,nauxret
      write(lunw_ret,*)version
      write(lunw_con,*)version
      do i=1,naddn
         write(lunw_ret,'(a)') addn_lines(i)(:lnbc(addn_lines(i)))
         write(lunw_con,'(a)') addn_lines(i)(:lnbc(addn_lines(i)))
      end do
...
c            if(spectrum_flag.eq.1) then
     ...
     &      dum,dum,dum,dum,dum,tins(iobs),pins(iobs),
...
c
            if(abs(asza_lav(iobs)-asza_ray).gt.0.)
     &      write(6,'(a,i4,2f10.4)')spectrum_ray(:24)//ray_string(:24)//
     &      ': Warning: ASZA mismatch ',iobs,asza_lav(iobs),asza_ray
c
c  Compute Latitude and longitude of tangent points
            if(asza_lav(iobs).le.90.) then
               zeff(iobs)=sngl(zobs(iobs))+scht*cos(d2r*asza_lav(iobs))
...
c
            ...
            write(*,*)'Warning: slantpaths are all =0 for ',spectrum_ray
...
c  Skip cell info
            do i=1,ncell
               read(lunr_mav,'(2f7.2,2e12.3)')z(1),t(1),p(1),d(1)
            end do
c  Read atmospheric level info
            do j=1,nlev_mav
               read(lunr_mav,'(2f7.2,2e12.3)')z(j),t(j),p(j),d(j)
            end do
         end do      !   spectrum_ray(4:8) .ne. next_spectrum_mav(4:8) 
c         write(6,'(i4,2x,a)') ocltn,specraywas(4:9)
         write(lunr_rpt,*)
         write(lunr_rpt,*) ocltn,specraywas(4:10)
         write(6,*)       'Number of different retrievd gases=',ngas
         write(lunr_rpt,*)'Number of different retrievd gases=',ngas
         ...
         write(6,*)       'Highest atmospheric tangent level =',hilay+1
         write(lunr_rpt,*)'Highest atmospheric tangent lavel =',hilay+1
         write(6,*)       'Number of retrieved levels        =',nlev_ray
         write(lunr_rpt,*)'Number of retrieved levels        =',nlev_ray
...
c  Read VMR file
         ...
         do ilev=1,mlev
            read(lunr_vmr,*,end=78)zvmr(ilev),
...
c  Start retrievals, gas by gas
         do kgas=1,ngas  ! Main Loop over retrieval gases
...
c  Also compute mean vmr (VBAR) for profile.
            do jgas=1,ncol_vmr-1
               ...
               if(c1.eq.'1' .or. c1.eq.'t') then
                  lavstarr(nauxlav+2*kgas-1)=
     &            lavstarr(nauxlav+2*kgas-1)(2:)
               endif
               call lowercase(gasnames(jgas+1))
               if(lavstarr(nauxlav+2*kgas-1).eq.gasnames(1+jgas)) exit
            end do
c            write(*,*) kgas,jgas,lavstarr(nauxlav+2*kgas-1),gasnames(jgas+1)
...
c                   write(*,*)'xx slerr=',xx,slerr(iobs,kgas)
                  pd1(iobs,j)=sngl(xx/slerr(iobs,kgas))
c                  if(slerr(iobs,kgas).gt.1.E+36)
c     &            write(*,*)' slerr.gt.1.0e+36',
c     &            iobs,kgas,j,xx,slerr(iobs,kgas),pd1(iobs,j),
c     &            specname,lavstarr(nauxlav+2*kgas-1)
               end do
               pd1(iobs,nl+1)=sngl(slcol(iobs,kgas)/slerr(iobs,kgas))
...
c  Multiply array PD1 by diurnal correction factors, if appropriate
            ...
            dcfile=gggdir(:lrt)//'diurnal/'//vmrfile(lvf-8:lvf-4)//
     &     '/dcf.'//dcflag//'.'//lavstarr(2*kgas+nauxlav-1)
            inquire(file=dcfile,exist=dcfilexst)
            if(dcfilexst) then
c               write(*,*) 'Found dcfile=',dcfile
               open(lunr_dc,file=dcfile,status='old')
               read(lunr_dc,*) nn, nsza
               do i=2,nn
                  read(lunr_dc,'(a)')dcflabel
               end do
c               call skiprec(lunr_dc,nn-2)
c               read(lunr_dc,'(a)')dcflabel
               ...
               do iobs=1,nobs
                  do j=1,nl
                     if(z(j).gt.zmin(iobs)) then
                        if(abs(zmin(iobs)-zobs(iobs)).le.0.) then  !  up looking geometry
                           sza=asin((zobs(iobs)+erad)*
     &                     sin(d2r*asza_lav(iobs))/(z(j)+erad))/d2r
                        else                   !  limb viewing geometry
                           sza=asin((zmin(iobs)+erad)/(z(j)+erad))/d2r
                        endif
                        dsunny=twodint(z(j),sza,talt,tsza,tab,malt,
     &                  nalt,nsza)
                        dnight=twodint(z(j),180-sza,talt,tsza,tab,malt,
     &                  nalt,nsza)
                        fr=sngl(zobs(iobs)-z(j))/(z(j+1)-z(j))
...
c               write(*,*) 'Not found dcfile=',dcfile
            endif   ! (dcfilexst)
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
c  Scale PD's by vbar to avoid overflows. 
            do iobs=1,nobs
               do j=1,nl
                  pd1(iobs,j)=pd1(iobs,j)*(abs(vaa(j))+sngl(vbar))/2
               end do
            end do
c - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
c   Duplicate entire array of partial differentials
            ...
            do iobs=1,neq
               ...
               write(lunw_jac,'(200(1pe12.4))')(pd1(iobs,j),j=1,nl+1)
            end do
c
...
c            write(*,*) 'lowlay=',lowlay,vmr(3,jgas),vmr(4,jgas),vmr(5,jgas)
            ...
            do ilay=1,nl
               xx=(abs(vaa(ilay))+sngl(vbar))/2
               vcon(ilay,kgas)=xx*vcon(ilay,kgas)
               vunc(ilay,kgas)=xx*vunc(ilay,kgas)
               ev(ilay,kgas)=
     &         xx*sngl(dsqrt(dsdot(nl,pd2(ilay,1),mm,pd2(ilay,1),mm)))
...
c               write(6,*)ta,tc,tu,tt
     ...
     &      verr,100*afit,100.*cfit,100*ufit,100*frac,smoo,dcflag
         end do    ! kgas=1,ngas   Main Loop over retrieval gases
c==================================================================
c  Write new VMR file
         ...
         do ilev=1,nlev_vmr
            write(lunw_vmr,'(f7.1,200(1pe11.3))') zvmr(ilev),
     &      (vmr(ilev,jgas),jgas=1,ncol_vmr-1)
         end do
         close(lunw_vmr)
c-----------------------------------------------------------------
...
c         write(*,*) 'lmin,lmax,ldel,z(lmin),z(lmax),delz= ',
c     &   lmin,lmax,ldel,z(lmin),z(lmax),delz
...
c  Interpolate YEAR, DAY, UT, ASZA, TLAT & TLON to the retrieval altitudes.
            ...
            aszai=0.0d0
            aszaz=0.0d0
c            write(*,*) ' ilay, z(ilay) = ',ilay,z(ilay+lowlay-1)
            do iobs=1,nobs
               dz=z(ilay+lowlay-1)-zeff(iobs)
               wt=exp(-0.5*(dz/delz)**2)
c               write(*,*)nint(z(ilay+lowlay-1)),iobs,sngl(year(iobs)),
c     &         sngl(zobs(iobs)),asza_lav(iobs),zmin(iobs),
c     &         zeff(iobs),sngl(dz),wt
               totwt=totwt+wt
               totwz=totwz+wt*dz
...
c            if(yeari.eq.2048.0) stop
            ...
            ndrec=ndrec+1
c            write(*,*)' ilay+lowlay-1 z = ',ilay+lowlay-1,z(ilay+lowlay-1)
