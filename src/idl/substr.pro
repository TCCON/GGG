...
;  2) Currently recognized delimiters include
;                       nul            (ASCII character # 0)
;                       horizontal tab (ASCII character # 9)
;                       space          (ASCII character # 32)
;                       comma          (ASCII character # 44)
;  3) Calls external functions FNBC.PRO (First Non-Blank Character),
...
;
     ...
     while ibeg gt iend  do begin
        ...
        nss=nss+1
     endwhile
     if nss gt mss then print, 'Warning from SUBSTR.PRO: Increase MSS to ',nss
     return
     end
