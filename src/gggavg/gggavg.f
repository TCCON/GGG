...
c                  the consistency of different windows of the same gas.
     ...
     & sew(mcol),escale(mcol),eybar(mrow),tew,totcon,
...
c           write(*,*) csformat
     ...
     &     sg,zlo,rmsfit,zmin,(airmass,ovcol,totcon,toterr,jtg=1,ktg)
           ...
           if(rmsfit.le.0.0) then
              write(*,*) 'Warning: rmsfit <= 0',colfile,irow
...
c          write(*,*)gas(iavg)
     ...
     $    ybar,eybar,scale(kavg),escale(kavg),sew(kavg),tew)
          twt=0.0d0
          tsc=0.0d0
          do icol=kavg,kavg+num-1
            write(lunt,'(a,3f9.5)') window(icol),scale(icol),
     $      escale(icol),sew(icol)
            weight=1.0d0/escale(icol)**2
            twt=twt+weight
     ...
     &         write(luno,'(3i5,a12,f9.5,a21,a,a12,a10)')iavg,irow,icol,
     &         ' Deviation =', error_term,' sigma for spectrum ',
     &          spectrum(irow)(:lnbc(spectrum(irow))),'  in window ',
     &          window(icol)
              endif
            end do
            yobs(iavg,irow)=ybar(irow)
            if(ybar(irow).eq.ymiss.and.eybar(irow).eq.ymiss) then
               yerr(iavg,irow)=ymiss
               nmiss_av=nmiss_av+1
            else
               yerr(iavg,irow)=dmax1(1.0d0,tew)*eybar(irow)
               nfound_av=nfound_av+1
            endif
          end do  ! irow=1,nrow
        else     ! num < 1
...
c
      ...
      write(lunw,'(2i3)') 6,nauxcol+2*navg
      write(lunw,'(a)') gggavg_version
      write(lunw,'(a)') gfit_version
      write(lunw,'(a)') gsetup_version
