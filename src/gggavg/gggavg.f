...
c
c  Addtionally, GGGAVG uses the subroutine TFIT to average sucessive windows
c  having the same target GAS, and also writes these averaged values along
c  with the auxiliary info, to an XYPLOT-readable spreadsheet file named
c  runlog.tav, runlog,lav, etc.
c
c  INPUT FILES:
c     runlog.xrl
...
c                  the consistency of different windows of the same gas.
      ...
      implicit none
      integer i1,iavg,idot,irow,j,jtg,k,kavg,ktg,lnbc,fnbc,fbc,
     & npp,lrl,l2,l3,l4,totnit,ntc,
     &lr,lunm,lunr,lunc,luno,lunt,lunv,lunw,mavg,mcol,ncol,icol,
     &mrow,nauxcol,navg,nlhead,nmiss_av,nmiss_sw,iyrwas,doywas,
     &nrow,nss,num,nit,mit,i,iyr,doy,istat,nfound_sw,nfound_av
      ...
      parameter (luno=16)
      parameter (lunv=17)
      parameter (mavg=50)     ! Max number of windows to be averaged
      parameter (mcol=600)    ! Total number of windows
      parameter (mrow=60000)  ! Max number of output records
      parameter (nauxcol=19)  ! Number of auxiliary parameters/columns

      character ans*1,apf*2,cl_av*1200,cdum*20,colabel*500,
     & gas(0:mavg)*8,gfit_version*80,gsetup_version*80,col_string*500,
     ...
     & colfile*40,gggavg_version*44,window(mcol)*10,spname_rlwas*21
     ...
     & sew(mcol),sigma_scale(mcol),sigma_ybar(mrow),tew,totcon,
     & toterr,ymiss,ybar(mrow),yaux(nauxcol,mrow),yerr(mcol,mrow),
     ...
     & snr,zenoff,zoff,sg,zpdwas,max_delta_t,delta_t,zmin
      ...
      gggavg_version=' GGGAVG    Version 2.6.6   09-Aug-2008   GCT'
      write(6,*) gggavg_version
...
c                 write(*,*)spname_rl(:lrl),delta_t, max_delta_t
     ...
     *    r8was,r8year,r8ydiff,iyr,doy,zpdtim
           r8was=r8year
           yaux(1,irow)=r8year
      ...
      do iavg=1,navg
        ...
        if(num.eq.1) then
          do irow=1,nrow
             ...
             if( yobs(iavg,irow).eq.ymiss .and.
     $       yerr(iavg,irow).eq.ymiss ) then
                nmiss_av=nmiss_av+1
             else
...
c          write(*,*)gas(iavg)
          call t_fit(ymiss,nrow,mcol,num,yobs(kavg,1),yerr(kavg,1),
     $    ybar,sigma_ybar,scale(kavg),sigma_scale(kavg),sew(kavg),tew)
          twt=0.0d0
          tsc=0.0d0
     ...
     $      sigma_scale(icol),sew(icol)
          ...
          do irow=1,nrow
            do icol=kavg,kavg+num-1
              if(yerr(icol,irow).ne.ymiss) then
                error_term=(yobs(icol,irow)-ybar(irow)*scale(icol))/
     $          (tew*yerr(icol,irow))
                if(abs(error_term).gt.5.)
     &          write(luno,'(a12,f9.5,a21,a,a12,a10)')' Deviation =',
     &          error_term,' sigma  for spectrum ',
     &          spectrum(irow)(:lnbc(spectrum(irow))),'  in window ',
     &          window(icol)
              endif
            end do
            yobs(iavg,irow)=ybar(irow)
            if(ybar(irow).eq.ymiss.and.sigma_ybar(irow).eq.ymiss) then
               yerr(iavg,irow)=ymiss
               nmiss_av=nmiss_av+1
            else
               yerr(iavg,irow)=dmax1(1.0d0,tew)*sigma_ybar(irow)
               nfound_av=nfound_av+1
            endif
          end do  ! irow=1,nrow
        else     ! num < 1
...
c  Write out average abundances to disk file
      ...
      write(6,'(i7,a)') nauxcol,' auxiliary columns'
      write(6,'(i7,a)') navg,' data columns (each value + error)'
      write(6,'(i7,a)') nrow,' data rows'
      write(6,'(i7,a,f5.1,a1)') nfound_av,' found values   = ',
     & 100*float(nfound_av)/(nfound_av+nmiss_av),'%'
      write(6,'(i7,a,f5.1,a1)') nmiss_av,' missing values = ',
