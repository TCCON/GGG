...
c  Writes out a version of the oof file but with the kitchen sink. 
     ...
     & aiafile*90,outfile*80,specname*38,tavheader*512,
     & vavheader*512,outfilec*80,gfit_version*80,gsetup_version*80,
     & adaheader*512,
     & inputfmt*1024,sp*1,adafmt*1024,
     & outfmt*1024,hdr(mhead)*(24),headaux(mhead)*(24),
     & corfmt*1024,ssss*9000,sarr(mrow)*38,csvarr(mrow)*38,
     & headtav(mhead)*(24),headvav(mhead)*(24),headada(mhead)*(24),
     & header*20000,gasname(mwin)*20,gggdir*80,corr_head*100,cc*38,
     & ooffmt*512,oof_header*8000,oof_out*800,aswfile*80,
     & col_header*20000,col_out*8000,headercsv*20000
      real*4 yauxt(maux),yobst(mwin),yerrt(mwin),yauxv(maux),
     & yobsv(mwin),yerrv(mwin),
     & yauxi(maux),yobsi(mwin),yerri(mwin)

      real*8 year,adcf(mwin),aicf(mwin)
      integer*4 oof_flag(mrow)
      integer*4 ncol, nchar, eflag, irow, nflag
      integer*4 kflag(mrow_qc), idontknowwhattocallthis_flag(mrow_qc), 
     & pindex(mrow_qc)
      ...
      we_version=
     &' write_eof  Version 0.2.1   2010-11-13   DW/CL'
      write(*,*) we_version
...
CC prepare oof
       ...
       call prepare_oof_output(aiafile,inputlun,
     & ooffmt, oof_flag, irow, 
     & vmin, vmax, pindex, 
     & nflag, eflag, kflag, idontknowwhattocallthis_flag,
     & nrow, ncol, nchar, scale, ofmt, oof_header)
CC
...
c Read and parse the corrections.dat file
      ...
      open(lun_cor,file=gggdir(:lnbc(gggdir))//'/tccon/corrections.dat',
     & status='old')
...
c Create the output header from the various file headers
     ...
     & (headtav(k)(:lnbc(headtav(k))+1),k=1,jj-naux-2),
     & (headvav(k)(:lnbc(headvav(k))+1),k=1,jj-naux-2),
     & (headada(k)(:lnbc(headada(k))+1),k=1,jj-naux-2),
...
c Set the formats of the files
      inputfmt='(a1,(a38,1x),f13.8,NNf13.5,800(1pe12.4))'
      write(inputfmt(20:21),'(i2.2)')naux-1

      adafmt='(a35,f14.8,NNf13.5,200(1pe12.4))'
      write(adafmt(12:13),'(i2.2)') naux-1
...
C end prepare files
     ...
     &                          percent_complete_target,"%"
          ...
          oof_out(:)=""
         call read_oneline_oof(aiafile,inputlun, oof_out,
     & ooffmt, oof_flag, irow,
     & vmin, vmax, pindex,
     & nflag, eflag, kflag, idontknowwhattocallthis_flag,
     ...
     &    //'aNNXX)'
         ...
         write(outfmt(NN6:NN6+3),'(i4.4)')lnbc(col_out)
