...
c
      implicit none
      integer*4
     &   kline1,kline2,jline,nlev,ncp,jcp,is,lnbc,lloc,llf,jtg,
...
c
c         do 7749 jline=kline1+1,kline2
...
c  Check that all linelist gases/species are in isotopomers.dat file
          ...
          write(*,*)'ABSCOG: Warning: unknown gas in linelist:', kgas
          write(*,*)'HITRAN contains unrecognized gases (O, NO+, HOBr)'
             cycle
          endif
     ...
     &       sqrt(abs(y*sxcgsorpidw/srpi/tnulst))))/godw)
             ...
             if(nv.gt.0) then
                ...
                do jv=kv1,kv2  ! Innermost loop over line profile
                   vac(jv,i,mm)=vac(jv,i,mm)-sxcgsorpidw*vv(jv)
                end do
             endif   ! nv.gt.0
          end do     ! i=1,nlev
