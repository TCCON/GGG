      subroutine rdopushead(path, iend, dtype, nsp, fxv, lxv, iy, im,
     & id,hh,mm,ss,ms,apt,nss,dur,vel,apf,phr,res,lwn,foc,nip,dfr,
...
c       foc,   R*8  ! Focal Length of telescope/collimating mirrors (mm).
...
c
      ...
      integer*4 dtype,hedlen,luns,one,mc,nss,gfw,gbw,iend,
     & bigendian,lnbc,lc
      integer*2 mrs,rs
      parameter (one=1,hedlen=512,luns=19,mrs=26,mc=26,
     & bigendian=1)
      integer*2 i2val(mrs)

      integer*4 nsp,iy,im,id,hh,mm,ss,ms,nip,dfr,pkl,prl,possp,i4val
      real*8 fxv, lxv, vel, phr, res, apt, lwn, dur, foc, r8val,
     & velocity(10),lfl,hfl
...
c            if (rs.gt.0) write(*,*)'rs,TIM1=',rs,hh,mm,ss,ms
...
c                read(cval(:4),*) ivel ! read velocity code value
c                read(cval(:4),'(i2)') ivel ! read velocity code value
                lc=lnbc(cval(:4))
...
c            if (rs.gt.0) write(*,*)'rs,TIM2=',rs,hh,mm,ss,ms
