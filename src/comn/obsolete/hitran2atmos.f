      subroutine hitran2atmos(kgas,kiso)
c  Converts a HITRAN gas number into its ATMOS value.
c
c  Inputs/Outputs:
c      KGAS = Gas Number
c      KISO = Isotopolog Number
c
c  Input arguments are overwritten by outputs
c  kgas=0 returned if gas is undefined in ATMOS linelist

      implicit none
      integer*4 kgas,kiso,mgas
      parameter (mgas=39)
      integer*4 llindex(mgas)
      data llindex/
c   ATMOS #     HITRAN #  Gas
     & 1,      !   1      H2O
     & 2,      !   2      CO2
     & 3,      !   3      O3
     & 4,      !   4      N2O
     & 5,      !   5      CO
     & 6,      !   6      CH4
     & 7,      !   7      O2
     & 8,      !   8      N2O
     & 9,      !   9      SO2
     & 10,     !  10      NO2
     & 11,     !  11      NH3
     & 12,     !  12      HNO3
     & 13,     !  13      OH
     & 14,     !  14      HF
     & 15,     !  15      HCl
     & 16,     !  16      HBr
     & 17,     !  17      HI
     & 18,     !  18      ClO
     & 19,     !  19      OCS
     & 20,     !  20      H2CO
     & 21,     !  21      HOCl
     & 41,     !  22      N2
     & 28,     !  23      HCN
     & 30,     !  24      CH3Cl
     & 23,     !  25      H2O2
     & 40,     !  26      C2H2
     & 38,     !  27      C2H6
     & 55,     !  28      PH3
     & 36,     !  29      COF2
     & 50,     !  30      SF6
     & 47,     !  31      H2S
     & 46,     !  32      HCOOH
     & 22,     !  33      HO2
     & 00,     !  34      O
     & 27,     !  35      ClONO2
     & 00,     !  36      NO+
     & 00,     !  37      HOBr
     & 39,     !  38      C2H4
     & 56/     !  39      CH3OH
c
c  H2O is a bit messy because ATMOS decided
c  to designate HDO as a separate molecule.
       if(kgas.le.0) then
          stop 'hitran2atmos: kgas.le.0'
       elseif(kgas.gt.mgas) then
          stop 'hitran2atmos: kgas.gt.mgas'
       elseif(kgas.eq.1 .and. kiso.ge.4) then
          kgas=49
          kiso=kiso-3
       else
          kgas=llindex(kgas)
       endif
       return
       end
