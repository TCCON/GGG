...
c  with any model for both ground-based and atmos geometries.
      ...
      rpsh=dlog(dble(p(lev2)/p(lev1)))/del_zlev
...
c         use snell's law in spherical geometry to evaluate true tangent ht.
          ...
          prdiff=riztan+(droc+ztan)*(1.0-riztan)*rpsh
...
8         b=388.0*ptan/ttan
          bend=-bend+b
          dbdt=-dbdt+1.72*b*(droc+zobs)*rpsh/xx
      endif
...
6     do 11 iter=1,2*maxiter
     ...
     & ifail)
      if(rsza.gt.90.0) then
      call getsp(nlev,z,p,t,180.0-rsza,roc,zobs,dum,opcon_tkr,spg,bendx,
     & ifail)
          bend=2.0*bend+bendx
      endif
      del_rsza=asza-rsza-bend
...
c  assuming no refraction above Ztoa
      th=dble(ths)*d2r
      if(zobs.gt.ztoa) then
          if(th.lt.piby2) return
          zh=dble(ztoa)
          zmin=droc*(dsin(th)-1.0d0) + dsin(th)*zobs
          zmin=dsin(th)*(droc+zobs)-droc
c          write(*,*)ths,th,dsin(th),zobs,ztoa,zmin
...
c
c  ds = integration step  < (layer thickness)/8, or 0.5 km horizontal.
      call find(nlev,z,sngl(zh),lev1,lev2)
      dx=0.5                    ! max allowed horizontal step 
c      dx=sqrt(2.5*ztoa*roc)/maxstep ! max allowed horizontal step 
...
c      write(*,*)'getsp:',ths,ds,dz,dx,dcos(th)
      ...
      thp=th+dt/2.
c
c  zhp & thp are the altitude and local zenith angle estimated for the middle
...
c
         ...
         zh=zh+dz
         th=th+dt
c         if(ths.eq.64.0) write(*,*) ths,th,zh,z(lev2),phi,dz
c
...
c  Precompute optical constant used to calculate refractive index gradient
      ...
      calc_opcon=aa*(aa+2.0d0)/(1.225014d0*(aa*(aa+2.0d0)+3.0d0))
      return
      end
c
...
c  rpsh = reciprocal scale height
      real*4 temp,pres
      real*8 opcon,d,grad,rpsh
      d=353.d0*pres*opcon/temp
c  refractive index = sqrt(1.0d0+2.0d0*d)/(1.0d0-d))
      grad=1.5*d*rpsh/((1.0d0+2.0d0*d)*(1.0d0-d))
      return
      end
