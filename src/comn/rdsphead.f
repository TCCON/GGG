     ...
     & apf,delwav,opd,fovi,snr,iy,im,id,gmt,lasf,wavtkr,lst,lse,lsu,lsf,
     & dip,mvd)
...
c      integer*2 i2val(mrs)
     ...
     & jj,
     & fftsiz,
     & posnrun,ic,krec,iflag,gfw,gbw,
     & lnbc,npoints,irec,reclen,lnblnk,
     & iend,nscan,iscan,nintp,ncenter,lst

      real*8 delwav,wsfact,dstep,nus,nue,nubar,
     & apt,dur,lfl,hfl,oblat,oblon,obalt,
     & wspd,wdir,sia,sis,vdc,lse,lsu,lsf,dip,mvd,
     & startf,stopf,res,phr,vel,foc,tgmt,
     & zpdtim,t_s,dt
...
c      idum=jd  !  Avoud compiler warning
...
c  Get the RUNLOG record and extract pertinent info
         open(19,file='/runlogs/atmos_runlog',access='direct',
     &   recl=hedlen)
...
c         tins=25.
c         hins=10.
         ifirst=int(nus/delwav+0.5)  ! subscript of first point in file
         if(abs(nus/delwav-ifirst) .gt. .04) then
            write(6,*)'RDSPHEAD Warning: Non-integral spectral grid'
            write(6,*)nus,nus/delwav
         endif
         ilast=int(nue/delwav+0.5)   ! subscript of first point in file
...
c         snr=1000.0d0
c
         dtype=5135 ! data_type (ratio spectrum?)
         dtype=1031 ! data_type (spectrum)
         c16=specname(16:17)
         if(c16.eq.'A.' .or. c16.eq.'B.' .or. c16.eq.'X.') dtype=2055 ! interferogram

         call read_opus_header(path(:lnblnk(path))//specname,iend,dtype,
     &   npoints,startf,stopf,
     &   iy,im,id,hh,mm,ss,ms,apt,dur,vel,apf,phr,res,lasf,foc,nip,dfr,
     &   pkl,prl,gfw,gbw,lfl,hfl,possp,oblat,oblon,obalt,tins,pins,hins,
     &   tout,pout,hout,wspd,wdir,sia,sis,vdc,lst,lse,lsu,lsf,dip,mvd,
     &   snr)
...
c
         ...
         ifirst=int(startf/delwav+0.5)
         ilast=int(stopf/delwav+0.5)
...
c  it was a FWD or REV scan. So adopt the following kludge.
         ...
         if(abs(hfl).le.0 .and. abs(lfl).le.0) hfl=lasf
         if(nip.le.0) then
            if(pkl.le.0 .or. prl.le.0) then
               nip=nint(2*0.9*(hfl-lfl)*(1/res+1/phr)/dfr)
            else
...
c  Positions in the (already opened) runlog to spectrum RUNLAB
      ...
      character specname*(*),specnamel*(nchr)
c
