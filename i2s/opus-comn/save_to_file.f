      subroutine save_to_file(datype,flimit,filename,channel,outfmt,
     & verbose,progver,mip,mif,mi4,mr8,buffer,counter,
     & minfreq,maxfreq,delimit,minmax,tpx,time,
     & i4head,r8head,DTCstr,INSstr,sivcfreq,pco_leni,pco_threshi,
     & izpd,sivcflag,dclevel,fvsi_calc,zpa,shbar,sherr,lsemode,
     & infovec,tlalevel, errnum)
c
c  Input:
...
c
     ...
     & tpx,        ! Subroutine input argument (see above)
     ...
     & INSstr*(*)   ! String to hold instrument description
      ...
      chanloc=channel      ! Avoids ftnchek warnings
      if(errnum.eq.0) then
        if(datype.eq.2) then
...
c             write(*,*)sivcfreq,minwav
          ...
          if(errnum.eq.0) then
            ...
            if(maxind.gt.counter) maxind=counter
     ...
     &         minwav,maxwav,minind,maxind,counter
        ...
        if(outfmt.eq.1) then
          call save_ascii(filename,verbose,mip,datype,buffer,counter,
     &     minfreq,maxfreq,minind,maxind,delimit,minmax,errnum)
        elseif(outfmt.eq.2) then
          call save_binary(filename,verbose,mip,buffer,
     &     minind,maxind,errnum)
        elseif(outfmt.eq.3) then
c        write(*,*)'Calling save_opus: minwav, maxwav=',minwav,maxwav
c        write(*,*)'Calling save_opus: minfreq, maxfreq=',minfreq,maxfreq
c        write(*,*)'Calling save_opus: minind, maxind=',minind,maxind
          call save_opus(filename,verbose,progver,mip,mif,mi4,mr8,
     &     datype,chanloc,buffer,counter,minfreq,maxfreq,minind,maxind,
     &    tpx,time,i4head,r8head,DTCstr,INSstr,sivcfreq,
     &    pco_leni,pco_threshi,izpd,
     &    sivcflag,dclevel,fvsi_calc,zpa,shbar,sherr,lsemode,
     &    infovec,tlalevel,errnum)
        endif    ! outfmt.eq.X
      endif
      return
...
c ===================================================================
      subroutine get_flimit(flimit,channel,errnum,minwav,maxwav)
c
c  Input:
c    flimit    C*(*)  Name of file containing the frequency limits
c    channel   I*4    Channel number (1=InGaAs=slave, 2=Si=master)
...
c
     ...
     & delimit*(*),! Subroutine input argument (see above)
     & stringx*13, ! String used to format X-value display
     & stringy*13  ! String used to format Y-value display
     ...
     &     path(1:lnbc(path))
      ...
      if(errnum.eq.0) then
        ...
        if((minmax.eq.0).and.(datype.eq.1)) then
          do indexa=minind,maxind
            if(errnum.eq.0) then
              write(stringx,'(i13)') indexa
              write(stringy,'(f10.7)') buffer(indexa)
              write(luna,'(3a)',iostat=ierr) stringx(fnbc(stringx):),
     &         delimit(1:1),stringy(fnbc(stringy):)
              if(ierr.ne.0) then
                errnum=-10
                write(*,'(a)')'Error: file write failed'
              endif
            endif
          enddo  ! indexa=minind,maxind
        elseif(minmax.eq.0) then
          do indexa=minind,maxind
            if(errnum.eq.0) then
              write(stringx,'(f13.8)')minfreq+(dble(indexa-1)*pointspac)
              write(stringy,'(f13.9)') buffer(indexa)
              write(luna,'(3a)',iostat=ierr) stringx(fnbc(stringx):),
     &         delimit(1:1),stringy(fnbc(stringy):)
              if(ierr.ne.0) then
                errnum=-10
          ...
          indexa=minind
          do while(indexa.le.maxind)
            first=indexa
            rmin=buffer(indexa)
            rmax=rmin
            indexa=indexa+1
            do while((indexa.le.maxind).and.
     &       ((indexa-first).lt.mmcnt))
              if(buffer(indexa).lt.rmin) rmin=buffer(indexa)
              if(buffer(indexa).gt.rmax) rmax=buffer(indexa)
              indexa=indexa+1
            enddo ! while((indexa.le.maxind).and.((indexa-....
            if(datype.eq.1) then
              if(errnum.eq.0) then
                write(stringx,'(i13)') (first+indexa)/2
                write(stringy,'(f10.7)') rmin
                write(luna,'(3a)',iostat=ierr) stringx(fnbc(stringx):),
     &           delimit(1:1),stringy(fnbc(stringy):)
                if(ierr.ne.0) then
                  errnum=-10
     ...
     &           delimit(1:1),stringy(fnbc(stringy):)
              ...
              if(errnum.eq.0) then
                write(stringx,'(f13.6)')
     &           minfreq+(dble((first+indexa-2)/2)*pointspac)
                write(stringy,'(f13.4)') rmin
                write(luna,'(3a)',iostat=ierr) stringx(fnbc(stringx):),
     &           delimit(1:1),stringy(fnbc(stringy):)
                if(ierr.ne.0) then
                  errnum=-10
...
c
c  If the file already exists, remove it.  Otherwise we would get an
c  incorrect file length if the new one is shorter than the old one.
c
        inquire(file=path,exist=filexist,iostat=ierr)
        if(ierr.ne.0) then
          errnum=-10
     ...
     &     path(1:lnbc(path))
        elseif(filexist) then
          if(verbose.ge.3) then
            write(*,'(2a)')'Pre-deleting file ',path(1:lnbc(path))
          endif
          open(unit=luna,file=path,iostat=ierr)
          if(ierr.ne.0) then
            errnum=-10
            write(*,'(2a)')'Error: pre-deleting failed on file ',
     &       path(1:lnbc(path))
          else
            close(unit=luna,status='delete',iostat=ierr)
            if(ierr.ne.0) then
              errnum=-10
              write(*,'(2a)')'Error: pre-deleting failed on file ',
     &         path(1:lnbc(path))
...
c
      if(errnum.eq.0) then
        open(unit=luna,file=path,access='direct',status='unknown',
...
c ===================================================================
     ...
     & datype,channel,buffer,counter,minfreq,maxfreq,minind,maxind,
     & tpx,time,i4head,r8head,DTCstr,INSstr,sivcfreq,
...
c
     ...
     & hfl         ! High folding limit, in cm-1
      ...
      character
     & path*(*),   ! Subroutine input argument (see above)
     & stringa*99, ! String used to format header items
...
c
        if(datype.eq.1) then
          ...
          fxv=minfreq+((dble(minind-1)*(maxfreq-minfreq))/dble(counter))
          lxv=minfreq+((dble(maxind-1)*(maxfreq-minfreq))/dble(counter))
        endif
c
...
c
        res=1.8d0*hfl/dble(tpx-izpd)
        phr=3.6d0*hfl/dble(pco_leni)
c
...
c        endif
     ...
     &     pointr,header)
          ...
          if(nint(infovec(35)).eq.0) then
            call pk_st_opus('ISS','OK',typ_string,iend,mhl,errnum,
     &       pointr,header)
          elseif(nint(infovec(35)).eq.1) then
            call pk_st_opus('ISS','Stress',typ_string,iend,mhl,errnum,
     &       pointr,header)
          elseif(nint(infovec(35)).eq.2) then
            call pk_st_opus('ISS','Error',typ_string,iend,mhl,errnum,
     &       pointr,header)
          elseif(nint(infovec(35)).eq.3) then
            call pk_st_opus('ISS','Reset',typ_string,iend,mhl,errnum,
     &       pointr,header)
          else
            call pk_st_opus('ISS','Unknown',typ_string,iend,mhl,errnum,
     &       pointr,header)
          endif
        endif
...
c
     ...
     &   pointr,header)
        ...
        if(tlalevel.ge.tla_ext_min) then
          call pk_r8_opus('OPL',dble(tpx-izpd)/(2.d0*hfl),iend,mhl,
     &     errnum,pointr,header)
          call pk_r8_opus('OPS',dble(izpd)/(2.d0*hfl),iend,mhl,
     &     errnum,pointr,header)
        endif
        call pk_i4_opus('NSS',
...
c
     ...
     &     pointr,header)
          call pk_r8_opus('PHR',phr,iend,mhl,errnum,pointr,header)
          call pk_r8_opus('SVF',sivcfreq,iend,mhl,errnum,pointr,header)
          call pk_i4_opus('PCL',pco_leni,iend,mhl,errnum,pointr,header)
        call pk_r8_opus('PCT',pco_threshi,iend,mhl,errnum,pointr,header)
          call pk_i4_opus('ZFF',i4head(i_zff),iend,mhl,errnum,
     &     pointr,header)

          if(tlalevel.ge.tla_ext_min) then
c In the OPUS format, the point indices are 0-based, hence "izpd-1" 
          call pk_i4_opus('ZPL',izpd-1,iend,mhl,errnum,pointr,header)
          call pk_i4_opus('SCF',sivcflag,iend,mhl,errnum,pointr,header)
          call pk_r8_opus('DCL',dclevel,iend,mhl,errnum,pointr,header)
          call pk_r8_opus('ZPA',zpa,iend,mhl,errnum,pointr,header)
          call pk_r8_opus('VER',progver,iend,mhl,errnum,pointr,header)
          call pk_r8_opus('VDC',fvsi_calc,iend,mhl,errnum,pointr,header)
          call pk_i4_opus('LST',lsemode,iend,mhl,errnum,pointr,header)
        call pk_r8_opus('LSE',dble(shbar),iend,mhl,errnum,pointr,header)
        call pk_r8_opus('LSU',dble(sherr),iend,mhl,errnum,pointr,header)
          endif
          call end_opus_prm(mhl,errnum,pointr,header)
          length=(pointr-bytstart)/4
...
c
     ...
     &     mhl,errnum,pointr,header)
        ...
        if(r8head(i_aptval).le.(9.95d0)) then
          write(stringa,'(f3.1,a3)')r8head(i_aptval),' mm'
        else
          write(stringa,'(f4.1,a3)')r8head(i_aptval),' mm'
        endif
...
c        if((i4head(i_dtccode).eq.1).and.(channel.eq.1)) then
c          call pk_st_opus('DTC','InGaAs-AC',typ_senum,iend,
c     &     mhl,errnum,pointr,header)
c        elseif((i4head(i_dtccode).eq.1).and.(channel.eq.2)) then
c          call pk_st_opus('DTC','Si-AC',typ_senum,iend,
c     &     mhl,errnum,pointr,header)
c        elseif((i4head(i_dtccode).eq.2).and.(channel.eq.1)) then
c          call pk_st_opus('DTC','InGaAs-DC',typ_senum,iend,
c     &     mhl,errnum,pointr,header)
c        elseif((i4head(i_dtccode).eq.2).and.(channel.eq.2)) then
...
c        endif
     ...
     &       pointr,header)
          ...
          elseif(i4head(i_pgn).eq.2) then             ! InSb, gain C
            call pk_i4_opus('PGR',25000,iend,mhl,errnum,pointr,header)
          elseif(i4head(i_pgn).eq.3) then             ! InSb, gain D = ref
            call pk_i4_opus('PGR',10000,iend,mhl,errnum,pointr,header)
          endif
        endif
...
c
     ...
     &     mhl,errnum,pointr,header)
        elseif(i4head(i_sfmcode).eq.sfm_idle) then
          call pk_st_opus('SFM','Short Scan for inactivity',typ_string,
     &     iend,mhl,errnum,pointr,header)
...
c
c      write(*,*)'pointr,minind,maxind=',pointr,minind,maxind
      if(errnum.eq.0) then
        open(unit=luna,file=path,form='unformatted',status='unknown',
...
c
     ...
     & bytloc(4)   ! Local copy of i4val as byte array
      ...
      elseif((bytpoint+4).ge.mhl) then
        ...
        i4loc=i4val
        if(iendian.eq.bigendian) call i4rev(i4loc)
        do indexa=1,4
          bytpoint=bytpoint+1
...
c
     ...
     & bytloc(8)   ! Local copy of r8val as byte array
      ...
      elseif((bytpoint+8).ge.mhl) then
        ...
        r8loc=r8val
        if(iendian.eq.bigendian) call r8rev(r8loc)
        do indexa=1,8
          bytpoint=bytpoint+1
          bythead(bytpoint)=bytloc(indexa)
        enddo
