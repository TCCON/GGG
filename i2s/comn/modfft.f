...
c      write (*,*) ' nfft not a power of 2 for ffak - stopped'
c      return

  20  n8pow = m/3

c  do a radix 2 or radix 4 iteration first if one is required
c      if (m-n8pow*3-1) 50, 40, 30        !  GCT 201203122
c  30  nn = 4
c      int = n/nn
c      call r4tr (int, b(1), b(int+1), b(2*int+1), b(3*int+1))
c      go to 60
c  40  nn = 2
c      int = n/nn
c      call r2tr (int, b(1), b(int+1))
c      go to 60
c  50  nn = 1
...
c  of the imaginary part of the intermediate results.
      ...
      l(1) = nn/8
      do 40 k=2,24
c        if (l(k-1)-2) 10, 20, 30        !  GCT 201203122
c  10    l(k-1) = 2
c  20    l(k) = 2
c        go to 40
c  30    l(k) = l(k-1)/2
...
c-----------------------------------------------------------------------
      ...
      n = 2**m
      do 40 j=4,n,2
c        if (k-j) 20, 20, 10       ! GCT 20120311
        ...
        k = k - 2
c        if (k-kl) 30, 30, 40       ! GCT 20120311
...
c        if (ij-ji) 30, 40, 40       ! GCT 20120311
...
c       write (*,*) ' nfft not a power of 2 for ffsk'
c        return
...
cc      if (m-n8pow*3-1) 90, 80, 70   ! GCT 20120311
c  70  int = n/4   !
c      call r4syn(int, b(1), b(int+1), b(2*int+1), b(3*int+1))
c      go to 90 
c  80  int = n/2 
c      call r2tr(int, b(1), b(int+1)) 
c  90  return 
...
c-----------------------------------------------------------------------
     ...
     & (l1,l(24))
      ...
      l(1) = nn/8
      do 40 k=2,24
c        if (l(k-1)-2) 10, 20, 30  !  GCT 20120311
c  10    l(k-1) = 2
c  20    l(k) = 2
c        go to 40
c  30    l(k) = l(k-1)/2
...
40    continue
      ...
      do 120 jthet=j23,l24,l23
        th2 = jthet - 2
c        if (th2) 50, 50, 90        !  GCT 201203122
        ...
        do 60 k=1,int
          ...
          br7(k) = t3 - t7
  60    continue
c        if (nn-8) 120, 120, 70        !  GCT 201203122
 ...
 100    continue
        jr = jr + 2
        ji = ji - 2
c        if (ji-jl) 110, 110, 120        !  GCT 201203122
