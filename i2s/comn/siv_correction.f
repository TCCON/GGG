      subroutine siv_correction(nip, bufigram, bufsmoo, frsp,
     &  pinl, pinv, sivcflag, dclevel, fvsi_calc, frzpda)
...
c
     ...
     & pinv,frsp,gthresh
      ...
      parameter (sthresh=0.01)

      real*8 dclevel,fvsi_calc,frzpda
...
c  Smooth DC igram (unshifted with respect to the original)
      call smooth_igram(nip,bufigram,bufsmoo,frsp,dclevel,frzpda,
     & pinv,pinl)

c       write(*,*) 'pinl,nip,nhw',pinl,nip
...
c  from the solar tracker
      ...
      smoo_std =0
c     write(42,*)bufsmoo
      smoo_mean=sum(bufsmoo)/nip
      do jip=1,nip
         smoo_std =smoo_std+(bufsmoo(jip)-smoo_mean)**2
      enddo
      smoo_std=sqrt(smoo_std/nip)
      fvsi_calc=smoo_std/smoo_mean
