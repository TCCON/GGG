       ...
       for my $j (0 .. $num_comm_col-1) {
           ...
           if ($previous[$i][$col1] ne $install[$m][$col2]) {
              if ((looks_like_number($previous[$i][$col1])) && (looks_like_number($install[$m][$col2]))) {
                 ...
                 if ($dd != 0 ) {
                  $count += 1;
                  print "Difference found at row=".($i+1)." col=".($col1+1)." column_name=$specname1[$col1] specname=$previous[$i][1]:\n";
...
if ($count == 0) {
   ...
   print "\tat [row=".($maxrow+1)." col=".($maxcol+1)."] or [$specname1[1]=$previous[$maxrow][1],column_name=$specname1[$maxcol]]\n";
   print "\n";
}
